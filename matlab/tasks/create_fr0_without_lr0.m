function create_fr0_without_lr0(conf,EXP,one_smru_name)
...
%     listatt = fieldnames(data_att);
    ...
    if contains(file_loc.name,'GPS')
        ...
        GPS_track=ncread([file_loc.folder '/' file_loc.name],'POS');
        seconde=datenum(0,0,0,0,0,1);
        date_GPS=datenum(GPS_start,'yyyy/mm/dd HH:MM:SS')+GPS_track(:,1)*seconde;
        GPS=[date_GPS,GPS_track(:,2:3)];
    elseif contains(file_loc.name,'ARGOS_SPOT')
        fid=fopen([file_loc.folder '/' file_loc.name])
        data2=textscan(fid,'%*q %*f %*s %s %*s %*s %f %f %*[^\n]',...
            'delimiter',',',...
            'Headerlines',1);
        fclose(fid);
        datemat3=datenum(data2{1,1});
        GPS=[datemat3,data2{1,2},data2{1,3}];
    else contains(file_loc.name,'ARGOS_CTD')
        fid=fopen([file_loc.folder '/' file_loc.name])
        data2=textscan(fid,'%s %f %f',...
            'delimiter',',',...
            'Headerlines',1);
        fclose(fid);
        datemat3=datenum(data2{1,1});
    ...
    [GPS]=correction_trajectoire(GPS);
    L=find(diff(GPS(:,1))<=0);
    if length(L)>0
        GPS(L,:)=[];
    end
    try
        ...
        lat_hr = interp1(GPS(:,1),GPS(:,2),jul_hr,'linear',NaN);
        lon_hr = interp1(GPS(:,1),GPS(:,3),jul_hr,'linear',NaN);
    catch
    ...
    % find bad location, for now we keep all TS profils without location
