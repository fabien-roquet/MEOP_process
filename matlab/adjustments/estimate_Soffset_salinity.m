function Soffset = estimate_Soffset_salinity(argo_qc,zone,P_comp)
% compare salinity at depth P_comp [default: 400m]
%             zone='RS': comparison in the Ross Sea

if nargin==2,
    P_comp=400; % Antarctic Continental Shelf
end
...
%         lon_bin_centre=lon_bin+.04;
    case 'RS' % Ross Sea
        lambdax=0.5;lambday=.25;lambdat=15;
        argo_qc.LONGITUDE(argo_qc.LONGITUDE<0)=argo_qc.LONGITUDE(argo_qc.LONGITUDE<0)+360;
        lat_bin=-78:.02:-73; lon_bin=160:.04:180;
        lat_bin_centre=lat_bin+.01;
        lon_bin_centre=lon_bin+.02;
end
...
for ii=1:argo_qc.ntag,
    ...
    K=find(strcmp(argo_qc.platform_number,descr)' & sum(argo_qc.PSAL_QC==1)>10);
    if length(K)>20,
        ...
        for kk=1:length(K),
            p=argo_qc.PRES(:,K(kk));
            s=argo_qc.PSAL(:,K(kk));
            I=find(~isnan(s)); p=p(I); s=s(I); 
            sm(kk) = interp1(p,s,P_comp,'linear');
        end
        Sm=[Sm; sm                      ];
    end

end
I=find(~isnan(Sm)); Nm=Nm(I); Xm=Xm(I); Ym=Ym(I); Tm=Tm(I); Sm=Sm(I);
