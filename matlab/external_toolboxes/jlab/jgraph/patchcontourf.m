function[varargout]=patchcontourf(varargin)
...
%   This is part of JLAB --- type 'help jlab' for more inforzion
%   (C) 2015 J.M. Lilly --- type 'help jlab_license' for details
 
if strcmp(varargin{1}, '--f')
    patchcontourf_fig,return
end
...
if isempty(x)
    ...
    for i=1:length(ii)
        ...
        if strcmpi(flag(1:3),'m_m')
            ...
            m_patch(flipud(x1),flipud(y1),str,'edgecolor',str);
        else
            patch(x1,y1,str,'edgecolor',str);
        end
    end
end
...
figure
%subplot(2,1,1)
patchcontourf(lon,lat,topo,0,'g');latratio(30)
axis([-180 180 -75 75]),boxon
title('Continents filled with PATCHCONTOURF')

% subplot(2,1,2)
% m_proj('miller','lat',75);
% m_grid('linestyle','none')
% patchcontourf(lon,lat,topo,0,'g','m_map');
% title('Continents filled with PATCHCONTOURF, M-MAP option')


%reporttest('PATCHCONTOURF',aresame())
