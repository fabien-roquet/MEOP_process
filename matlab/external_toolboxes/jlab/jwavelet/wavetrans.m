...
%   ___________________________________________________________________
%
%   See also MORSESPACE, RIDGEWALK, SPHERETRANS, WAVESPECPLOT.
%
...
if isempty(goodindex)
    ...
    T=wavetrans_continue(zeros(size(x(:,1))),argcell);
    T=inf*(1+sqrt(-1)).*vrep(T,size(x,2),2);
else
    x=x(:,goodindex);
    T1=wavetrans_continue(x,argcell);
...
function[T]=wavetrans_continue(x,argcell)
