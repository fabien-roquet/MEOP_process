...
% can use only the optimization toolbox
warning off
...
for Iprofile = 1:number_profiles
    ...
    pindex = 1:2:mp;
    [Inn] = find(~isnan(SA_in(pindex,Iprofile) + t(pindex,Iprofile) + p(pindex,Iprofile)));
    ...
    SA_tmp = SA_in(pindex(Inn),Iprofile);
    t_tmp = t(pindex(Inn),Iprofile);
    p_tmp = p(pindex(Inn),Iprofile);
    ...
    opts = optimoptions(@lsqlin,'Algorithm','interior-point',...
        'LinearSolver','sparse','Display','off');
    x =  lsqlin(H, f, A, b_U, [], [], x_L, [],[],opts);
    SA_tmp = SA_tmp + x;
    
    %--------------------------------------------------------------------------
    pindex2 = pindex(Inn(1)):pindex(Inn(end));
    SA_out(pindex2,Iprofile) = interp1(p_tmp,SA_tmp,p(pindex2,Iprofile));
...
% 
% USAGE:  
%  [N2, N2_alpha, N2_beta, dSA, dCT, dp] = ...
%                                    gsw_Nsquared_min_const_t(SA,t,p)
%
% DESCRIPTION:
