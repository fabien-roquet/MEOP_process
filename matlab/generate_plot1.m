...
for itag = 1:length(info_deployment.list_tag)
    ...
    if exist(name_prof,'file'),
        ...
        if ~isempty(Mqc.PRES)
            Mgroup=ARGO_concat_TS(Mgroup,Mqc);
        else
            continue;
        end
    else
...
for itag = 1:length(info_deployment.list_tag)
    ...
    L = find(strcmp(Mgroup.list_descr,Mgroup.list_platform_number{itag}));
    if length(L)>=2 && ~strcmp(Mgroup.list_smru_platform{itag}, smru_name)
        for kk = L
            I=(sum(Mgroup.nprof(1:kk-1))+1:sum(Mgroup.nprof(1:kk)))';
            ...
            if ~isempty(smru_name) && contains(smru_name,'N1')
                smru_name = Mgroup. list_smru_platform{itag+1};
            else
                smru_name = Mgroup. list_smru_platform{itag};
            end
    ...
    elseif strcmp(Mgroup.list_smru_platform{itag}, smru_name)
        continue
    else
        I=(sum(Mgroup.nprof(1:L-1))+1:sum(Mgroup.nprof(1:L)))';
        if length(I)==length(Mgroup.index_tag)
            Mtemp=Mgroup;
        elseif ~isempty(I)
            Mtemp=extract_profil(Mgroup,I);
