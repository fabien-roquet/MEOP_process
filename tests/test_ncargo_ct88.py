...
def test_create_ncargo_python_writes_ct88_lr0_core_file(meop_config, stage_ct88_example) -> None:
    ...
    result = create_ncargo_python(
        ...
        now=datetime(2024, 3, 6, 2, 15, 16, tzinfo=timezone.utc),
    )
    ...
    with _open_any(staged["reference_lr0"]) as reference, _open_any(candidate) as created:
        assert int(created.sizes["N_PROF"]) == 306
        assert int(created.sizes["N_LEVELS"]) == 16
...
def test_ct88_core_subset_matches_reference_dataset(meop_config, stage_ct88_example) -> None:
    staged = stage_ct88_example()
    assert import_raw_data_zip(meop_config, "ct88") is True
...
def test_ct88_reference_comparison_matches_qc_and_profile_counts_after_filter_port(meop_config, stage_ct88_example) -> None:
    staged = stage_ct88_example()
    assert import_raw_data_zip(meop_config, "ct88") is True
    create_ncargo_python(
        meop_config,
    ...
    report = compare_netcdf_outputs(
        staged["reference_lr0"],
        candidate,
        attributes=["number_of_ts_profiles", "number_of_t_profiles"],
        variables=["PRES_QC", "TEMP_QC", "PSAL_QC"],
    )
