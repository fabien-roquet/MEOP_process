...
def test_cli_forwards_keep_intermediate_override(meop_config, seed_catalog, monkeypatch) -> None:
    seed_catalog(deployment="DEP001", smru_name="DEP001-AAA")
    captured: list[dict[str, object]] = []

    def fake_process_tags(**kwargs):
        captured.append(kwargs)
        return True

    monkeypatch.setattr(cli_module, "process_tags", fake_process_tags)

    exit_code = cli_module.main(
        ["--deployment", "DEP001", "--process_data", "--keep-intermediate-products", "--no-diagnostics"],
        config=meop_config,
    )

    assert exit_code == 0
    assert captured[0]["keep_intermediate_products"] is True
...
def test_cli_forwards_batch_keep_intermediate_override(meop_config, monkeypatch) -> None:
    captured: list[dict[str, object]] = []
    ...
    exit_code = cli_module.main(
        ["--run-all-deployments", "--keep-intermediate-products"],
        config=meop_config,
    )
