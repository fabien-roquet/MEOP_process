...
\fancyhead[R]{\textcolor{meopmid}{April 1, 2026}}
...
\begin{titlepage}
  ...
  \begin{tabularx}{0.93\textwidth}{>{\bfseries}p{0.30\textwidth}X}
    ...
    Tests in repository & 46 collected tests in the current snapshot \\
  \end{tabularx}
...
The most important practical change in this phase is operational rather than architectural: there is now a resumable batch runner suitable for reprocessing all available deployments. It keeps a persistent per-deployment state file, skips deployments that already completed successfully unless forced, records one log file per deployment, generates readable Markdown and CSV run reports, and refreshes \pkg{list\_tags.csv} and \pkg{list\_deployments.csv} incrementally at the end of the run.
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item a runtime data manager that owns where tables, catalogs, raw data, reference data, outputs, and batch state live;
\end{enumerate}
...
Runtime data layout and validation & Python & Explicit directories under \pkg{data/}, \pkg{references/}, and output roots. \\
...
Location adjustment & Placeholder & Left explicit for later scientific work. \\
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item processed netCDF outputs;
  \item plots and diagnostics;
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Mirrored metadata JSON:} \pkg{data/config\_files/}, for example \pkg{deployment2.json}, \pkg{platform2.json}, and patch files.
  \item \textbf{Low-resolution raw inputs:} \pkg{data/raw\_smru\_data\_odv/}.
  \item \textbf{High-resolution raw inputs:} \pkg{data/raw\_smru\_hr\_data/<year>/}, containing files such as \pkg{<instr\_id>\_ctd.txt}.
  \item \textbf{Reference datasets:} \pkg{references/}, including WOD, CORA, the last stable MEOP release, and shortened regression fixtures.
  \item \textbf{Processed outputs:} \pkg{final\_dataset\_prof/}, containing \pkg{lr0}, \pkg{lr1}, \pkg{hr0}, \pkg{hr1}, \pkg{hr2}, \pkg{fr0}, \pkg{fr1}, and \pkg{traj} products.
  \item \textbf{Diagnostics:} \pkg{plots/}.
  \item \textbf{Batch state and reports:} \pkg{data/batch/}.
\end{itemize}

\subsection{How raw data are expected to be found}

Low-resolution data are staged directly under \pkg{data/raw\_smru\_data\_odv/}. High-resolution files are resolved from \pkg{list\_deployment\_hr.csv} and are expected under the runtime HR tree, typically as files named like \pkg{<optional\_prefix><instr\_id>\_ctd.txt} inside the appropriate year directory.
...
\begin{itemize}[leftmargin=1.5em]
  \item \pkg{deployment\_status.json}: persistent deployment-level state;
  ...
  \item \pkg{latest/}: copies of the most recent report and state.
\end{itemize}

A deployment marked successful is skipped on the next batch run unless a force option is used.
...
The repository currently contains 46 collected tests covering:
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item cleanup of any remaining migration-era compatibility behavior that should eventually disappear once validation is complete.
\end{itemize}
