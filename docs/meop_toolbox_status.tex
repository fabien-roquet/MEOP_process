...
\fancyhead[R]{\textcolor{meopmid}{April 23, 2026}}
...
\begin{titlepage}
  ...
  \begin{tabularx}{0.93\textwidth}{>{\bfseries}p{0.30\textwidth}X}
    ...
    Pipeline now covered in Python & Catalog discovery, raw ODV import (including FL sidecar merging), \pkg{lr0}, QC/filtering, \pkg{hr0}, \pkg{hr1/lr1}, \pkg{fr0}, \pkg{fr1}, \pkg{hr2}, standard diagnostics, CORA-based T/S calibration plots, and resumable batch processing \\
    Execution model & Single Python execution path; no alternate runtime backend remains in the package \\
    Tests in repository & 150 collected tests in the current snapshot \\
  \end{tabularx}
...
CORA-based T/S calibration plots & Python & Loaded via \pkg{meop-compare --plot1 <smru\_name>}; two-panel figure per 200-profile chunk (T/S diagram + PSAL anomaly). \\
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item CORA-based T/S calibration plots (\pkg{meop-compare --plot1}).
\end{itemize}
...
The repository currently contains 150 collected tests covering:
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item CORA tile loader (bounding-box filtering, multi-tile concatenation, empty-tile handling);
  \item CORA-based T/S calibration plot generation, chunking, and \pkg{meop-compare --plot1} CLI;
